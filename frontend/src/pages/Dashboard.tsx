import {
  ...
  ArrowRight, Calendar, CheckCircle2, XCircle
} from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={payrollTrend}>
              ...
              <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid #e5e7eb' }} formatter={(v: any) => [`$${Number(v).toLocaleString()}`, 'Payroll']} />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            { label: 'Add Employee', icon: UserPlus, color: 'bg-blue-50 text-blue-600', page: 'employees' as const },
            { label: 'Mark Attendance', icon: Clock, color: 'bg-green-50 text-green-600', page: 'attendance' as const },
            { label: 'Apply Leave', icon: Calendar, color: 'bg-orange-50 text-orange-600', page: 'leaves' as const },
            { label: 'Run Payroll', icon: DollarSign, color: 'bg-violet-50 text-violet-600', page: 'payroll' as const },
            { label: 'Post Job', icon: Briefcase, color: 'bg-pink-50 text-pink-600', page: 'recruitment' as const },
            { label: 'View Reports', icon: TrendingUp, color: 'bg-cyan-50 text-cyan-600', page: 'reports' as const },
          ].map(action => (
            <button
              key={action.label}
