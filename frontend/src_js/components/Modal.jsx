...
export default function Modal({ isOpen, onClose, title, subtitle, children, size = 'md', footer }) {
    ...
    return (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className={`bg-white rounded-2xl w-full ${sizes[size]} max-h-[90vh] flex flex-col animate-scale-in`} onClick={(e) => e.stopPropagation()}>
        ...
        {footer && (<div className="p-6 border-t border-gray-100 flex items-center justify-end gap-3">
            {footer}
          </div>)}
      </div>
    </div>);
