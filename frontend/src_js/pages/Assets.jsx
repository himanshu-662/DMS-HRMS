...
import { Package, Laptop, Monitor, Smartphone, Plus, Search, CheckCircle2, Wrench, AlertTriangle } from 'lucide-react';
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search assets..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary-500/30"/>
        </div>
        <select value={filterType} onChange={e => setFilterType(e.target.value)} className="px-3 py-2.5 bg-gray-50 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary-500/30">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(asset => {
            ...
            return (<div key={asset.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg hover:shadow-gray-100/50 transition-all group">
              ...
              <div className="mt-3 space-y-1.5">
                {asset.assignedTo && (<div className="flex items-center gap-2 text-xs text-gray-500">
                ...
                {asset.assignedDate && (<div className="text-[11px] text-gray-400">
              ...
              <div className="mt-4 pt-3 border-t border-gray-100 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                {asset.status === 'available' ? (<Button size="sm" fullWidth onClick={() => { setSelectedAsset(asset); setShowAssignModal(true); }}>
                    Assign
                  </Button>) : asset.status === 'assigned' && (<Button size="sm" variant="secondary" fullWidth onClick={() => handleUnassign(asset)}>
                    Unassign
                  </Button>)}
              </div>
            </div>);
      ...
      {/* Add Asset Modal */}
      <Modal isOpen={showAddModal} onClose={handleCloseModal} title="Add New Asset" subtitle="Add a new asset to the inventory" footer={<>
            <Button variant="ghost" onClick={handleCloseModal}>Cancel</Button>
            <Button onClick={handleAddAsset}>Add Asset</Button>
          </>}>
        <div className="space-y-4">
          <FormInput label="Asset Name" value={formData.name} onChange={(v) => setFormData({ ...formData, name: v })} placeholder="MacBook Pro 16" required/>
          <div className="grid grid-cols-2 gap-4">
            <FormSelect label="Type" value={formData.type} onChange={(v) => setFormData({ ...formData, type: v })} options={[
            ...
            <FormSelect label="Condition" value={formData.condition} onChange={(v) => setFormData({ ...formData, condition: v })} options={[
          ...
          <FormInput label="Serial Number" value={formData.serialNumber} onChange={(v) => setFormData({ ...formData, serialNumber: v })} placeholder="SN-2024-001" required/>
        </div>
      </Modal>

      {/* Assign Modal */}
      <Modal isOpen={showAssignModal} onClose={() => { setShowAssignModal(false); setSelectedAsset(null); setAssignTo(''); }} title="Assign Asset" subtitle={`Assign ${selectedAsset?.name} to an employee`} footer={<>
        ...
        <FormSelect label="Assign to Employee" value={assignTo} onChange={setAssignTo} options={[
