import { Users, UserCheck, Clock, CalendarX, DollarSign, UserPlus, Briefcase, TrendingUp, ArrowRight, Calendar, CheckCircle2, XCircle } from 'lucide-react';
...
export default function Dashboard() {
    ...
    return (<div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={attendanceChartData} barGap={2}>
              ...
              <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid #e5e7eb', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}/>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-2">
            {deptDistribution.map(d => (<div key={d.name} className="flex items-center gap-1.5 text-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {pendingLeaves.length === 0 ? (<p className="text-sm text-gray-500 text-center py-4">No pending leave requests</p>) : (pendingLeaves.slice(0, 4).map(leave => (<div key={leave.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80 hover:bg-gray-100/80 transition-colors">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => handleApproveLeave(leave.id)} className="w-7 h-7 rounded-lg bg-success-50 text-success-600 flex items-center justify-center hover:bg-success-500 hover:text-white transition-colors">
                      <CheckCircle2 className="w-4 h-4"/>
                    </button>
                    <button onClick={() => handleRejectLeave(leave.id)} className="w-7 h-7 rounded-lg bg-danger-50 text-danger-600 flex items-center justify-center hover:bg-danger-500 hover:text-white transition-colors">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            ...
            return (<div key={candidate.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80 hover:bg-gray-100/80 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        ].map(action => (<button key={action.label} onClick={() => navigateTo(action.page)} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-100 hover:border-primary-200 hover:shadow-md hover:shadow-primary-100/50 transition-all group">
