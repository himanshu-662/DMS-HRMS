...
import { Search, Plus, Download, Upload, Mail, MapPin, Building2, Grid3X3, List, Eye, Edit, Trash2, AlertCircle, LayoutGrid, GripVertical, FileUp } from 'lucide-react';
...
export default function Employees() {
    ...
    const onDragEnd = async (result) => {
        ...
        if (!employee)
            return;
        const updatedEmployees = employees.map(e => e.id === employeeId ? { ...e, department: newDepartment } : e);
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search by name, email, or ID..." value={searchQuery} onChange={e => dispatch({ type: 'SET_SEARCH', payload: e.target.value })} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:bg-white transition-all"/>
          </div>
      ...
      {viewMode === 'grid' && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((emp, idx) => (<div key={emp.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg hover:shadow-gray-100/50 transition-all group">
      ...
      {viewMode === 'group' && (<DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-6 overflow-x-auto pb-6 h-[calc(100vh-280px)] min-h-[500px]">
            {departments.map((dept, deptIdx) => (<div key={dept} className="flex-shrink-0 w-80 flex flex-col gap-4">
                ...
                <Droppable droppableId={dept}>
                  {(provided, snapshot) => (<div {...provided.droppableProps} ref={provided.innerRef} className={cn("flex-1 p-2 rounded-2xl border-2 border-dashed transition-all space-y-3 overflow-y-auto", snapshot.isDraggingOver ? "bg-primary-50/50 border-primary-200" : "bg-gray-50/30 border-transparent")}>
                      {groupedEmployees[dept].map((emp, index) => (<Draggable key={emp.id} draggableId={emp.id} index={index}>
                          {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} className={cn("bg-white p-4 rounded-xl border border-gray-100 shadow-sm transition-shadow", snapshot.isDragging ? "shadow-xl ring-2 ring-primary-500/20 rotate-1" : "hover:shadow-md")}>
      ...
      {/* View Modal */}
      <Modal isOpen={showViewModal} onClose={handleCloseModal} title="Employee Details" size="lg" footer={<><Button variant="ghost" onClick={handleCloseModal}>Close</Button><Button onClick={() => { handleCloseModal(); if (selectedEmployee)
        handleEdit(selectedEmployee); }}>Edit</Button></>}>
