...
export default function Leaves() {
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-gray-100">
        <div className="flex items-center justify-between px-5 pt-4 border-b border-gray-100">
          <div className="flex items-center gap-1">
            {['requests', 'balances', 'calendar'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={cn('px-4 py-2.5 text-sm font-medium rounded-t-xl transition-colors border-b-2 -mb-px', activeTab === tab
        ...
        {activeTab === 'requests' && (<div>
            <div className="px-5 py-3 flex items-center gap-2 border-b border-gray-50">
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-3 py-2 bg-gray-50 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary-500/30">
            ...
            <div className="divide-y divide-gray-50">
              {filtered.length === 0 ? (<div className="p-12 text-center">
                ...
                </div>) : (filtered.map(leave => {
                const config = statusConfig[leave.status];
                const StatusIcon = config.icon;
                return (<div key={leave.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/50 transition-colors">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        {leave.status === 'pending' && (<div className="flex items-center gap-1">
                            <button onClick={() => handleApprove(leave.id)} className="w-8 h-8 rounded-xl bg-success-50 text-success-600 flex items-center justify-center hover:bg-success-500 hover:text-white transition-colors">
                              <CheckCircle2 className="w-4 h-4"/>
                            </button>
                            <button onClick={() => handleReject(leave.id)} className="w-8 h-8 rounded-xl bg-danger-50 text-danger-600 flex items-center justify-center hover:bg-danger-500 hover:text-white transition-colors">
        ...
        {/* Balances Tab */}
        {activeTab === 'balances' && (<div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {leaveBalances.map(balance => {
                const usedPercentage = (balance.used / balance.total) * 100;
                return (<div key={balance.type} className="border border-gray-100 rounded-2xl p-4 hover:shadow-md transition-all">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2 mb-3">
                    <div className={cn('h-2 rounded-full transition-all', usedPercentage > 80 ? 'bg-red-500' : usedPercentage > 50 ? 'bg-amber-500' : 'bg-primary-500')} style={{ width: `${usedPercentage}%` }}/>
                  </div>
        ...
        {/* Calendar Tab */}
        {activeTab === 'calendar' && (<div className="p-5">
            ...
            ].map(h => (<div key={h.date} className="bg-white rounded-xl p-3 border border-gray-100">
      ...
      {/* Apply Leave Modal */}
      <Modal isOpen={showApplyModal} onClose={handleCloseModal} title="Apply for Leave" subtitle="Submit a new leave request" footer={<>
            <Button variant="ghost" onClick={handleCloseModal}>Cancel</Button>
            <Button onClick={handleSubmit}>Submit Request</Button>
          </>}>
        <div className="space-y-4">
          <FormSelect label="Leave Type" value={formData.type} onChange={(v) => setFormData({ ...formData, type: v })} options={[
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormInput label="Start Date" type="date" value={formData.startDate} onChange={(v) => setFormData({ ...formData, startDate: v })} required error={errors.startDate} min={new Date().toISOString().split('T')[0]}/>
            <FormInput label="End Date" type="date" value={formData.endDate} onChange={(v) => setFormData({ ...formData, endDate: v })} required error={errors.endDate} min={formData.startDate || new Date().toISOString().split('T')[0]}/>
          </div>
          {formData.startDate && formData.endDate && (<div className="p-3 rounded-xl bg-primary-50 text-primary-700 text-sm">
              <strong>{calculateDays(formData.startDate, formData.endDate)}</strong> day(s) selected
            </div>)}
          <FormTextarea label="Reason" value={formData.reason} onChange={(v) => setFormData({ ...formData, reason: v })} placeholder="Please provide a reason for your leave..." required error={errors.reason}/>
        </div>
      </Modal>
    </div>);
}
