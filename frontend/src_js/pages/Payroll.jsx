...
import { DollarSign, Download, CheckCircle2, Clock, TrendingUp, Eye, Play } from 'lucide-react';
...
export default function Payroll() {
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={payrollBreakdown} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3} dataKey="value">
                {payrollBreakdown.map((entry, i) => (<Cell key={i} fill={entry.color}/>))}
              </Pie>
              <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid #e5e7eb' }} formatter={(v) => `$${Number(v).toLocaleString()}`}/>
            </PieChart>
          </ResponsiveContainer>
          <div className="space-y-2">
            {payrollBreakdown.map(item => (<div key={item.name} className="flex items-center justify-between text-sm">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="px-3 py-2 bg-gray-50 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary-500/30">
              ...
              {stats.pending > 0 && (<Button size="sm" icon={<Play className="w-4 h-4"/>} onClick={() => setShowProcessConfirm(true)}>
                  Process All
                </Button>)}
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {payroll.map(record => (<tr key={record.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={cn('text-[11px] font-semibold px-2.5 py-1 rounded-full', record.status === 'processed' ? 'bg-emerald-100 text-emerald-700' : record.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600')}>
                        {record.status.toUpperCase()}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setShowPayslip(record.id)} className="w-8 h-8 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-primary-600 transition-colors">
                          <Eye className="w-4 h-4"/>
                        </button>
                        {record.status === 'pending' && (<button onClick={() => handleProcessPayroll(record.id)} className="w-8 h-8 rounded-lg hover:bg-success-50 flex items-center justify-center text-gray-400 hover:text-success-600 transition-colors">
      ...
      {/* Payslip Modal */}
      <Modal isOpen={!!selectedPayslip} onClose={() => setShowPayslip(null)} title="Payslip" subtitle={selectedPayslip?.month} footer={<>
      ...
      <ConfirmDialog isOpen={showProcessConfirm} onClose={() => setShowProcessConfirm(false)} onConfirm={handleProcessAll} title="Process All Payroll" message={`This will process payroll for ${stats.pending} pending records. This action cannot be undone.`} type="success" confirmText="Process All"/>
    </div>);
}
