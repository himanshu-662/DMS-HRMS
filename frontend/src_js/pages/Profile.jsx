...
import { Mail, Phone, MapPin, Building2, Calendar, Briefcase, Award, Shield, Edit, Camera, Save } from 'lucide-react';
...
export default function Profile() {
    ...
    return (<div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        ].map(item => (<div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80">
        ...
        ].map((job, idx) => (<div key={idx} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={cn('w-3 h-3 rounded-full flex-shrink-0 mt-1.5', job.current ? 'bg-primary-500' : 'bg-gray-300')}/>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {['HR Strategy', 'Recruitment', 'Employee Relations', 'HRIS', 'Compliance', 'Performance Management', 'Training', 'Payroll'].map(skill => (<span key={skill} className="text-xs font-medium px-3 py-1.5 rounded-full bg-primary-50 text-primary-700 border border-primary-100">
                  {skill}
                </span>))}
            </div>
          </div>
        ...
        ].map(stat => (<div key={stat.label} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
      ...
      {/* Edit Profile Modal */}
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Profile" subtitle="Update your personal information" footer={<>
