...
import { Briefcase, Users, UserCheck, Plus, MapPin, Clock, Star, ChevronRight, Eye, Trash2 } from 'lucide-react';
...
export default function Recruitment() {
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-gray-100">
        <div className="flex items-center justify-between px-5 pt-4 border-b border-gray-100">
          <div className="flex items-center gap-1">
            {['jobs', 'candidates', 'pipeline'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={cn('px-4 py-2.5 text-sm font-medium rounded-t-xl transition-colors border-b-2 -mb-px', activeTab === tab
        ...
        {/* Jobs Tab */}
        {activeTab === 'jobs' && (<div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {jobPostings.map(job => (<div key={job.id} className="border border-gray-100 rounded-2xl p-5 hover:shadow-lg hover:shadow-gray-100/50 transition-all group">
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center">
                    <Briefcase className="w-5 h-5 text-primary-600"/>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={cn('text-[11px] font-semibold px-2.5 py-1 rounded-full', job.status === 'open' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600')}>
                      {job.status.toUpperCase()}
                    </span>
                    <button onClick={() => { setSelectedJob(job); setShowDeleteConfirm(true); }} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-danger-600 opacity-0 group-hover:opacity-100 transition-all">
        ...
        {/* Candidates Tab */}
        {activeTab === 'candidates' && (<div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {candidates.map(c => (<tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <select value={c.stage} onChange={(e) => handleMoveCandidate(c.id, e.target.value)} className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border-0 cursor-pointer ${stageColors[c.stage]}`}>
                        {[...stages, 'rejected'].map(s => (<option key={s} value={s}>{stageLabels[s]}</option>))}
                      </select>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      {c.rating > 0 ? (<div className="flex items-center gap-1">
                          <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400"/>
                          <span className="text-sm font-medium text-gray-700">{c.rating}</span>
                        </div>) : (<span className="text-xs text-gray-400">N/A</span>)}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => { setSelectedCandidate(c); setShowCandidateModal(true); }} className="w-8 h-8 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-primary-600 transition-colors ml-auto">
        ...
        {/* Pipeline Tab */}
        {activeTab === 'pipeline' && (<div className="p-5">
            <div className="flex gap-4 overflow-x-auto pb-4">
              {stages.map(stage => {
                const stageCandidates = candidates.filter(c => c.stage === stage);
                return (<div key={stage} className="min-w-[240px] flex-shrink-0">
                    ...
                    <div className="space-y-2 min-h-[200px]">
                      {stageCandidates.map(c => (<div key={c.id} className="bg-white border border-gray-100 rounded-xl p-3 hover:shadow-md transition-all cursor-pointer" onClick={() => { setSelectedCandidate(c); setShowCandidateModal(true); }}>
                          ...
                          {c.rating > 0 && (<div className="flex items-center gap-1">
                              {[1, 2, 3, 4, 5].map(star => (<Star key={star} className={cn('w-3 h-3', star <= Math.floor(c.rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200')}/>))}
                              <span className="text-[10px] text-gray-400 ml-1">{c.rating}</span>
                            </div>)}
                          {stage !== 'onboarding' && stage !== 'selected' && (<div className="flex gap-1 mt-2">
                              <button onClick={(e) => {
                                e.stopPropagation();
                                const nextStage = stages[stages.indexOf(stage) + 1];
                                if (nextStage)
                                    handleMoveCandidate(c.id, nextStage);
                      ...
                      {stageCandidates.length === 0 && (<div className="text-center py-8 text-xs text-gray-400 border-2 border-dashed border-gray-100 rounded-xl">
                          No candidates
                        </div>)}
                    </div>
                  </div>);
      ...
      {/* Post Job Modal */}
      <Modal isOpen={showJobModal} onClose={handleCloseJobModal} title="Post New Job" subtitle="Create a new job opening" size="lg" footer={<>
            <Button variant="ghost" onClick={handleCloseJobModal}>Cancel</Button>
            <Button onClick={handleCreateJob}>Post Job</Button>
          </>}>
        <div className="space-y-4">
          <FormInput label="Job Title" value={jobForm.title} onChange={(v) => setJobForm({ ...jobForm, title: v })} placeholder="Senior Frontend Developer" required/>
          <div className="grid grid-cols-2 gap-4">
            <FormSelect label="Department" value={jobForm.department} onChange={(v) => setJobForm({ ...jobForm, department: v })} options={['Engineering', 'Design', 'Product', 'Marketing', 'Sales', 'Finance', 'HR', 'Operations'].map(d => ({ value: d, label: d }))}/>
            <FormSelect label="Job Type" value={jobForm.type} onChange={(v) => setJobForm({ ...jobForm, type: v })} options={[
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormInput label="Location" value={jobForm.location} onChange={(v) => setJobForm({ ...jobForm, location: v })} placeholder="San Francisco, CA" required/>
            <FormInput label="Experience Required" value={jobForm.experience} onChange={(v) => setJobForm({ ...jobForm, experience: v })} placeholder="3-5 years"/>
          </div>
          <FormTextarea label="Job Description" value={jobForm.description} onChange={(v) => setJobForm({ ...jobForm, description: v })} placeholder="Describe the role, responsibilities, and requirements..." rows={4}/>
        </div>
      </Modal>

      {/* Candidate Details Modal */}
      <Modal isOpen={showCandidateModal} onClose={() => { setShowCandidateModal(false); setSelectedCandidate(null); }} title="Candidate Details" size="lg" footer={<Button onClick={() => { setShowCandidateModal(false); setSelectedCandidate(null); }}>Close</Button>}>
        {selectedCandidate && (<div className="space-y-4">
              ...
              <div className="p-3 rounded-xl bg-gray-50">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  {selectedCandidate.rating > 0 ? (<>
                      {[1, 2, 3, 4, 5].map(star => (<Star key={star} className={cn('w-4 h-4', star <= Math.floor(selectedCandidate.rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200')}/>))}
            ...
            {selectedCandidate.stage !== 'onboarding' && selectedCandidate.stage !== 'rejected' && (<div className="flex gap-2 pt-4 border-t border-gray-100">
      ...
      <ConfirmDialog isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} onConfirm={handleDeleteJob} title="Delete Job Posting" message={`Are you sure you want to delete "${selectedJob?.title}"? This action cannot be undone.`} type="danger" confirmText="Delete"/>
    </div>);
}
