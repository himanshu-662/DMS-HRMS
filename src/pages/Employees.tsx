...
import {
  ...
  Mail, MapPin, Building2, Grid3X3, List, Eye, Edit, Trash2, FileJson, FileSpreadsheet, FileText, AlertCircle, LayoutGrid, GripVertical, FileUp
} from 'lucide-react';
...
const emptyEmployee: Omit<Employee, 'id' | 'employeeId'> = {
  name: '',
  email: '',
  phone: '',
...
type ImportType = 'json' | 'csv' | 'excel';
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <Modal isOpen={showAddModal} onClose={handleCloseModal} title={isEditing ? 'Edit Employee' : 'Add New Employee'} subtitle={isEditing ? 'Update employee information' : 'Fill in the details to add a new team member'} size="lg" footer={<><Button variant="ghost" onClick={handleCloseModal}>Cancel</Button><Button onClick={handleSubmit}>{isEditing ? 'Update' : 'Add Employee'}</Button></>}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4"><FormSelect label="Employment Type" value={formData.type} onChange={(v) => setFormData({ ...formData, type: v as Employee['type'] })} options={[{ value: 'full_time', label: 'Full Time' }, { value: 'part_time', label: 'Part Time' }, { value: 'contract', label: 'Contract' }, { value: 'intern', label: 'Intern' }]} /><FormInput label="Join Date" type="date" value={formData.joinDate} onChange={(v) => setFormData({ ...formData, joinDate: v })} /></div>
